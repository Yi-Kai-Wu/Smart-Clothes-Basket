...
void moveScrew(byte group) {
  ...
  while (!finish_sort) {
    ...
    if (Nema1.currentPosition() == leftNEMA && Nema2.currentPosition() == leftNEMA) {
      finish_sort = true; // Set finish flag to true when movement is complete
      sorting = false;
    }
  }
}
...
void readTagOperation() {
  displayString("Group 6: Clothes Sort", "-------- SORT --------", "Searching for tag ...", "");
    ...
    if (r200Serial.available()) {
      ...
      if (start == 0xAA && command == 0x02) {   // EPC tag read
        ...
        if (foundGroup == 0x01) {
          ...
          displayString("Group 6: Clothes Sort", "-------- SORT --------", "1. Personal clothing", "");
          delay(250);
        } else if (foundGroup == 0x02) {
          Serial.println("Tag found in group: 2. light colored");
          displayString("Group 6: Clothes Sort", "-------- SORT --------", "2. Light colored", "");
          delay(250);
        } else if (foundGroup == 0x03) {
          Serial.println("Tag found in group: 3. dark colored");
          displayString("Group 6: Clothes Sort", "-------- SORT --------", "3. Dark colored", "");
          delay(250);
        } else {
          Serial.println("Tag not found in any group");
          displayString("Group 6: Clothes Sort", "-------- SORT --------", "# Not in any group #", "");
          delay(250);
        }  
        printAllTags();
        sorting = true;
...
State currentState = STATE_SORT;
State nextState = STATE_SORT;
State previousState = STATE_SORT;
